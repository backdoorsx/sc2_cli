...
class Sc2API:
    ...
    def query_streams(idx):
        ...
        for data in query_data:
            ...
            try:
                movie_data.append((data['ident'],data['size'],data['video'],data['audio'],data['subtitles']))
            except KeyError:
                print('[-] Key Error!')
...
class Core:
    ...
    def name(ver):
        ...
        if r == 0:
            print('    Kodi cli tip: Use enter for YES or AUTO\n')
        if r == 1:
            print('    Kodi cli tip: [AUTO] = Automatically found the best option in your language\n')
    ...
    def find_movie(tag):
        ...
        print('')
        print(f'{SN}99) Back to Main menu')
        print('')

        all_data = []
        
        if tag == 'movie':
            name = input(f'{SN}Find movie > ')                          # INPUT
        else:
            name = None                                             # INPUT
        ...
        if name == '99':
            ...
            for i in range(len(datax)):
                ...
                print(f'{SN}'+'+'+76*'-')
                if i >= 10:
                    print('{}|  {}  |{}|  {}  |  {}'.format(SN, datax[i][0], seen, datax[i][1], datax[i][2]))
                else:
                    print('{}|  {}   |{}|  {}  |  {}'.format(SN, datax[i][0], seen,  datax[i][1], datax[i][2]))

            print(f'{SN}'+'+'+76*'-'+'\n')
            print('')
            print(f'{SN}back) Back to Find menu')
            print('')

            choose = input(f'{SN}Choose number: ')                                  # INPUT
    ...
    def find_serial():
        ...
        while True:
            ...
            print(f'{SN}99) Back to Main menu')
            print('')
            name = input(f'{SN}Find series > ')                              # INPUT
            all_data = []
            ...
            if name == '99':
                ...
                datax_reverse = datax.copy()
                datax_reverse.reverse()

                for i in range(len(datax_reverse)):
                    print(f'{SN}'+'+'+76*'-')
                    if i >= 10:
                        print('{}|  {}  |  {}  |  {}'.format(SN, datax_reverse[i][0], datax_reverse[i][1], datax_reverse[i][2]))
                    else:
                        print('{}|  {}   |  {}  |  {}'.format(SN, datax_reverse[i][0], datax_reverse[i][1], datax_reverse[i][2]))

                print(f'{SN}'+'+'+76*'-'+'\n')
                print('')
                print(f'{SN}99) Back to the Find series menu')
                print('')
                    ...
                    if choose == 99:
                        ...
                        if all_data == 'episode':                       # serial nema serie
                            ...
                            for i in range(len(all_data)):
                                print(f'{SN}'+'+'+76*'-')
                                print('{}|  {}  |  {}  |  {}'.format(SN, i, all_data[i][1], all_data[i][2]))
                            ...
                            season = input(f'{SN}{datax[choose][1]} | Choose number > ') # INPUT
                            ...
                            if season == 99:
                                ...
                                for i in range(len(e_data)):
                                    ...
                                    if i < 10-1:
                                        ...
                                        spacer = ''
                                    print('{}|{}  {}   |{}|  {}  |  {} {}'.format(SN, spacer, i+1, seen, e_data[i][1], e_data[i][2], waiting))
                                ...
                                episode = input('\n{}{} | Season{} | Choose number [{}-{}] > '.format(SN, datax[choose][1], all_data[season][2],1 ,len(e_data) )) # INPUT
                                    
                                if episode.isnumeric():
                                    episode = int(episode)
                                    episode-=1 #indexovanie od nuly, zobrazenie pre uzivatela od jednotky
                                    
                                    if episode == 99:
                                        print('')
                                        continue
                                    elif episode >= len(e_data):
                                        input(f'[-] Episode {episode+1} is not available!')
                                    else:
                                        print(f'\n[DEBUG] {e_data[episode]}\n')
        ...
        for s in range(len(streams_data)):                          # ident size video audio
            ...
            if len(stream_isva[2]) > 0:
                ...
                for audio in stream_isva[3]:
                    ...
                    #audio_data += ' '
                    if audio['language'].upper() not in audio_data:
                        audio_data += audio['language'].upper()
                        audio_data += '/'
                audio_data = 'Zvuk: ' + audio_data[:-1]

                sub_data = ''
                for subtitles in stream_isva[4]:
                    try:
                        if subtitles['language'].upper() not in sub_data:
                            sub_data += subtitles['language'].upper()
                            sub_data += '/'
                    except KeyError:
                        sub_data += 'NaN/'
                sub_data = 'Titulky: ' + sub_data[:-1]
                ...
                print('{}| {}{}\t{}\t{}'.format(SN, space, s, quality, size))
                print('{}| {}\t{}\t{}\t{}'.format(SN, empty, video_width, audio_data, sub_data))

                if LABEL_LANG.upper() in audio_data:
                    found_best_in_lang = str(s)

        print(f'{SN}'+'+'+76*'-')
        print('')
        print(f'{SN}99) Back to the Main menu')
        print('')
        
        while True:

            if found_best_in_lang.isnumeric():
                stream_choose = input(f'{SN}{movie_name} [AUTO = {found_best_in_lang}] > ')      # INPUT
            else:
                stream_choose = input(f'{SN}{movie_name} > ')      # INPUT

            if stream_choose.isnumeric():
                stream_choose = abs(int(stream_choose))
                ...
                if stream_choose == 99:
                    ...
                    return 0
            elif stream_choose == '' and found_best_in_lang.isnumeric():
                found_best_in_lang = int(found_best_in_lang)
                ident_stream = streams_data[found_best_in_lang][0]
                Core.link(ident_stream, movie_name, streams_data[found_best_in_lang])
                return 0
    ...
    def link(ws_ident, movie_name, selected_streams_data):
        ...
        if os.path.exists(KODI_CLI_FILE):
            ...
            try:
                ...
                login = 'FAIL'
                print(f'[-] Connection failed! Check webshare site or login for user {username}')
    ...
    def player(link):
        ...
        if os.name == 'nt':
            ...
            try:
                ...
                print(f'[-] Dont run video player {VIDEO_PLAYER} player!')
                print('')
    ...
    def add_db(data):
        ...
        if not os.path.exists(KODI_CLI_FILE2):
            print(f'[-] Not found {KODI_CLI_FILE2}')
        else:
    ...
    def settings():
        ...
        try:
            setting_menu = int(input(f'{SN}settings > '))       # INPUT
    ...
    def set_login():
        print('')
        print(f'{SN}Login for webshare.cz. Set user name and password.')
        print('')
        print(f'{SN}99) Back to the Main menu')
        print('')
        ...
        try:
            login_input = input(f'{SN}settings:login > webshare username: ')    # INPUT USER

            if len(login_input) != 0 and login_input != '99':
                ws_username = login_input

                login_input = input(f'{SN}settings:login > webshare passwod: ') # INPUT PASS
                if len(login_input) != 0 and login_input != '99':
                    ws_pass = login_input
    ...
    def hide(u,p):
        ...
        try:
            ...
            f.write(gen)
            f.close()
            print(f'\n[+] Save shadow login data to {KODI_CLI_FILE} OK\n')
        except:
            print(f'\n[-] Failed save shadow login data to {KODI_CLI_FILE} OK\n')
            ...
            if menu == '0':
                ...
                while True:
                    ...
                    while n<len(all_episode):
                        ...
                        nxt = input(f'{SN}Next episode {all_episode[n][2]} ? [yes/no/retry] > ')
                        print(serial_name)
                        if nxt.upper() in ['', 'YES', 'Y', 'YOP', 'A', 'ANO']:
                            ...
                            n+=1
                        elif nxt.upper() in ['R', 'RE', 'RET', 'RETR', 'RETRY']:
                            n-=1
                            e_idx = all_episode[n][0]
                            _data = Sc2API.query_streams(e_idx)
                            serial_name_split = serial_name.split('Episode')
                            serial_name = serial_name_split[0] + 'Episode' + str(int(serial_name_split[1])-1)
                            print(serial_name_split)
                            Core.sort(serial_name, _data)
                            n+=1
                        elif nxt.upper() in ['N', 'NO', 'NIE', 'NOP', 'NE', '99']:
                            break

                    print(f'{SN}End season!')
                        
            elif menu == '5':
                db_json = Core.read_db()
                print(len(db_json))
                input('db > ')
                for db_item in db_json:
                    print('')
                    print(db_item)
                input('db > ')
            elif menu == '99':
