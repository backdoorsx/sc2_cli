...
class Sc2API:
    def query_search(search_name, tag_type):
        ...
        #DONE #https://plugin.sc2.zone/api/media/filter/news?type=movie&sort=dateAdded&order=desc&days=365&access_token=th2tdy0no8v1zoh1fs59 MOVIE
        #https://plugin.sc2.zone/api/media/filter/v2/news?type=tvshow&sort=dateAdded&order=desc&days=365&access_token=th2tdy0no8v1zoh1fs59 SERIAL
        
        if tag_type == 'new':
            QUERY_URL_API = "https://plugin.sc2.zone/api/media/filter/news?type=movie&sort=dateAdded&order=desc&days=365&access_token={}".format(TOKEN)
            ...
            try:
                if len(i2) != 0:
                    if len(i2) == 1:                                                    # iba jeden jazyk
                        searched.append((n, i2[0]["title"], get_year, d["_id"]))        # asi stale cz
                    elif len(i2) >= 3:                                                  # 3 a viac jazyky
                        if i2[2]["lang"] == 'en' and i2[2]["title"] != '':
                            searched.append((n, i2[2]["title"], get_year, d["_id"]))    # en
                        elif i2[1]["title"] != '':
                            searched.append((n, i2[1]["title"], get_year, d["_id"]))    # sk
                        else:
